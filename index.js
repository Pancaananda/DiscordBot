...
try {
  ...
  console.log(
    "No .env file found or dotenv error. Using Replit Secrets instead.",
  );
}
...
if (!process.env.DISCORD_TOKEN) {
  console.error(
    "DISCORD_TOKEN is not set! Please add it to your Replit Secrets.",
  );
  process.exit(1);
}
...
if (!process.env.YOUTUBE_API_KEY) {
  console.error(
    "YOUTUBE_API_KEY is not set! Please add it to your Replit Secrets.",
  );
  process.exit(1);
}
...
const loadChannels = () => {
  try {
    if (fs.existsSync(DATA_FILE)) {
      ...
      console.log(
        `Loaded ${youtubeChannels.length} YouTube channels from file.`,
      );
    } else {
...
async function checkYouTubeLiveStatus(channelId) {
  try {
    ...
    console.error(
      `Error checking YouTube live status for ${channelId}:`,
      error,
    );
    return null;
  }
...
client.on("messageCreate", async (message) => {
  ...
  if (command === "addchannel") {
    ...
    if (!identifier) {
      return message.reply(
        "Invalid YouTube channel URL. Please provide a valid YouTube channel URL (e.g., youtube.com/@username or youtube.com/channel/ID).",
      );
    }
    ...
    try {
      ...
      const existingChannel = youtubeChannels.find(
        (c) => c.channelId === channelId,
      );
      if (existingChannel) {
        return message.reply(
          `This YouTube channel is already being monitored in ${existingChannel.notificationChannelId ? `<#${existingChannel.notificationChannelId}>` : "a channel"}.`,
        );
      }
  ...
  if (command === "removechannel") {
    ...
    if (args.length < 1) {
      return message.reply(
        "Please provide a YouTube channel URL or channel name.",
      );
    }
    ...
    if (identifier) {
      ...
      try {
        ...
        if (channelId) {
          ...
          youtubeChannels = youtubeChannels.filter(
            (c) => c.channelId !== channelId,
          );
    ...
    if (!removed) {
      const initialLength = youtubeChannels.length;
      youtubeChannels = youtubeChannels.filter(
        (c) => !c.channelName.toLowerCase().includes(query.toLowerCase()),
      );
      removed = youtubeChannels.length < initialLength;
    }
    ...
    if (removed) {
      ...
      return message.reply(
        "Could not find this YouTube channel in the monitoring list.",
      );
    }
  }
  ...
  if (command === "listchannels") {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        "These channels will trigger notifications when they go live:",
      )
      .addFields(
      ...
      .addFields([
        {
          ...
          value:
            "Add a YouTube channel to monitor for livestreams (Admin only)",
        },
        {
